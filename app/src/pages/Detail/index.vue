                    ...
                    <div class="choose">
                        <div class="chooseArea">
                            ...
                            <dl v-for="attr in spuSaleAttrList" :key="attr.id">
                                ...
                                <dd changepirce="0" @click="changeChecked(attrValue,attr.spuSaleAttrValueList)" :class="{active:attrValue.isChecked==='1'}" v-for="attrValue in attr.spuSaleAttrValueList" :key="attrValue.id">{{attrValue.saleAttrValueName}}</dd>
                            </dl>
                        </div>
                        <div class="cartWrap">
                            <div class="controls">
                                <input autocomplete="off" class="itxt" v-model.number="count" @change="changeCount"/>
...
export default {
    ...
    computed:{
        ...
        ...mapGetters("detail",["valuesSkuJson","categoryView","spuSaleAttrList","skuInfo"])
    },

    mounted(){
        this.$store.dispatch("detail/getDetail",this.$attrs.skuId)
    },

    methods:{
        // 改变购买信息（颜色、版本）
        changeChecked(attrValue,spuSaleAttrValueList){
            spuSaleAttrValueList.forEach((item,index) => {
                item.isChecked = "0";
        ...
        // 加入购物车
        async addShopCart(){
            // 1.发请求：将产品数据加入到数据库（通知服务器）
            // 2.服务器存储成功：进行路由跳转传递参数
            // 3.失败：给用户提示

            // await 后面是Promise，返回的是Promise的结果
            let result;
            try {
                result = await this.$store.dispatch("detail/addOrUpdateShopCart",{skuId:this.$route.params.skuId,skuNum:this.count});
                // 将对象转换为JSON存储到本地
